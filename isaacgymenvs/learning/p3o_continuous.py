...
def actor_loss(old_action_neglog_probs_batch, action_neglog_probs, advantage, is_ppo, curr_e_clip, cost_advantage, penalty):
    if is_ppo:
        ...
        a_loss = (action_neglog_probs * advantage)
        a1_loss = (action_neglog_probs * cost_advantage)

    return a_loss, a1_loss
...
class PPOLagBase(BaseAlgorithm):
    ...
    def write_stats(self, total_time, epoch_num, step_time, play_time, update_time, a_losses, a1_losses, c_losses, cost_losses, entropies, kls, last_lr, lr_mul, frame, scaled_time, scaled_play_time, curr_frames):
        ...
        self.writer.add_scalar('losses/ac_loss', torch_ext.mean_list(a1_losses).item(), frame)
        self.writer.add_scalar('losses/c_loss', torch_ext.mean_list(c_losses).item(), frame)
        self.writer.add_scalar('losses/cost_loss', torch_ext.mean_list(cost_losses).item(), frame)
    ...
    def get_stats_weights(self, model_stats=False):
        ...
        if model_stats:
            ...
            if self.normalize_value:
                ...
                state['costv_mean_std'] = self.model.costv_mean_std.state_dict()
    ...
    def set_stats_weights(self, weights):
        ...
        if self.normalize_value and 'normalize_value' in weights:
            ...
            self.model.costv_mean_std.load_state_dict(weights['costv_mean_std'])
    ...
    def play_steps(self):
        ...
        for n in range(self.horizon_length):
            ...
            shaped_costs = self.rewards_shaper(infos["cost"])

            if self.value_bootstrap and 'time_outs' in infos:
                shaped_rewards += self.gamma * res_dict['values'] * self.cast_obs(infos['time_outs']).unsqueeze(1).float()
                shaped_costs += self.gamma * res_dict['costvs'] * self.cast_obs(infos['time_outs']).unsqueeze(1).float()
            ...
            self.current_costs = self.current_costs * not_dones.unsqueeze(1)
            self.current_lengths = self.current_lengths * not_dones

        last_values, last_costvs = self.get_values(self.obs)

        fdones = self.dones.float()
        mb_fdones = self.experience_buffer.tensor_dict['dones'].float()
        mb_values = self.experience_buffer.tensor_dict['values']
        mb_rewards = self.experience_buffer.tensor_dict['rewards']
        mb_costvs = self.experience_buffer.tensor_dict['costvs']
        mb_costs = self.experience_buffer.tensor_dict['costs']       
        mb_advs = self.discount_values(fdones, last_values, mb_fdones, mb_values, mb_rewards)
        mb_cost_advs = self.discount_values(fdones, last_costvs, mb_fdones, mb_costvs, mb_costs)
        mb_returns = mb_advs + mb_values
        mb_cost_returns = mb_cost_advs + mb_costvs

        batch_dict = self.experience_buffer.get_transformed_list(swap_and_flatten01, self.tensor_list)
        batch_dict['returns'] = swap_and_flatten01(mb_returns)
        batch_dict['cost_returns'] = swap_and_flatten01(mb_cost_returns)
...
class ContinuousPPOLagBase(PPOLagBase):
    ...
    def train_epoch(self):
        ...
        for mini_ep in range(0, self.mini_epochs_num):
            ...
            for i in range(len(self.dataset)):
                a_loss, a1_loss, c_loss, cost_loss, penalty_loss, entropy, kl, last_lr, lr_mul, cmu, csigma, b_loss = self.train_actor_critic(self.dataset[i])
                a_losses.append(a_loss)
                a1_losses.append(a1_loss)
        ...
        return batch_dict['step_time'], play_time, update_time, total_time, a_losses, a1_losses, c_losses, cost_losses, b_losses, penalty_losses, entropies, kls, last_lr, lr_mul

    def prepare_dataset(self, batch_dict):
        obses = batch_dict['obses']
        returns = batch_dict['returns']
        cost_returns = batch_dict['cost_returns']
        dones = batch_dict['dones']
        values = batch_dict['values']
        costvs = batch_dict['costvs']
        ...
        cost_advantages = cost_returns - costvs
        ...
        if self.normalize_value:
            ...
            self.costv_mean_std.train()
            costvs = self.costv_mean_std(costvs)
            cost_returns = self.costv_mean_std(cost_returns)
        ...
        dataset_dict['old_costvs'] = costvs
        dataset_dict['old_logp_actions'] = neglogpacs
        dataset_dict['advantages'] = advantages
        dataset_dict['cost_advantages'] = cost_advantages
        dataset_dict['returns'] = returns
        dataset_dict['cost_returns'] = cost_returns
    ...
    def train(self):
        ...
        while True:
            ...
            step_time, play_time, update_time, sum_time, a_losses, a1_losses, c_losses, cost_losses, b_losses, penalty_losses, entropies, kls, last_lr, lr_mul = self.train_epoch()
            total_time += sum_time
            ...
            if self.rank == 0:
                ...
                self.write_stats(total_time, epoch_num, step_time, play_time, update_time, a_losses, a1_losses, c_losses, cost_losses, entropies, kls, last_lr, lr_mul, frame, scaled_time, scaled_play_time, curr_frames)
                if len(b_losses) > 0:
...
class P3OAgent(ContinuousPPOLagBase):
    ...
    def calc_gradients(self, input_dict):
        ...
        cost_preds_batch = input_dict['old_costvs']
        old_action_log_probs_batch = input_dict['old_logp_actions']
        advantage = input_dict['advantages']
        cost_advantage = input_dict['cost_advantages']
        old_mu_batch = input_dict['mu']
        old_sigma_batch = input_dict['sigma']
        return_batch = input_dict['returns']
        cost_return_batch = input_dict['cost_returns']
