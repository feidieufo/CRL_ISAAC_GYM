...
class ModelPPOLagContinuousLogStd(BaseModel):
    def __init__(self, network):
        BaseModel.__init__(self, 'ppolag')
        self.network_builder = network


    class Network(BaseModelNetwork):
        def __init__(self, ppolag_network, **kwargs):
            BaseModelNetwork.__init__(self, **kwargs)
        ...
        def get_default_rnn_state(self):
            return self.ppolag_network.get_default_rnn_state()

        def forward(self, input_dict):
            is_train = input_dict.get('is_train', True)
            prev_actions = input_dict.get('prev_actions', None)
            input_dict['obs'] = self.norm_obs(input_dict['obs'])
            mu, logstd, value, cost, states = self.ppolag_network(input_dict)
